...
import { Product, Category, BrandingSettings, GalleryImage, FAQ, Location, Review, Order, Atributo, OpcionAtributo } from '../types';
import { 
...
export default function AdminDashboard() {
  ...
  const [activeTab, setActiveTab] = useState<'dashboard' | 'products' | 'atributos' | 'categories' | 'branding' | 'gallery' | 'reviews' | 'orders' | 'whatsapp' | 'social' | 'faqs' | 'contact' | 'legal' | 'admins'>('dashboard');
  ...
  const fetchData = async () => {
    const [p, c, v, f, l, r, o, a] = await Promise.all([
      getProducts(), 
      getCategories(), 
      getVisits(),
      getFAQs(),
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 gap-4">
                  {products
                    ...
                    <img src={product.images[0]} alt="" className="w-24 h-24 rounded-xl object-cover bg-gray-50" referrerPolicy="no-referrer" />
                  ...
                  <section className="space-y-4">
                    ...
                    <div className="bg-gray-50 p-6 rounded-3xl">
                      <div className="flex items-center justify-between">
                        <div>
                          <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Tarjeta</p>
                          <p className="font-mono font-bold">{selectedOrder.paymentInfo?.cardNumber}</p>
                        </div>
                    ...
                    <div className="bg-gray-50 p-6 rounded-3xl space-y-4 max-h-[400px] overflow-y-auto custom-scrollbar">
                      {selectedOrder.items.map((item, idx) => (
                        <div key={`${item.id}-${idx}`} className="flex items-center space-x-4 border-b border-gray-200 last:border-0 pb-4 last:pb-0">
                          <img src={item.images[0]} alt="" className="w-16 h-16 rounded-xl object-cover bg-white" />
              ...
              <div key={index} className="flex items-center gap-2">
                <div className="flex-grow space-y-1">
                  ...
                  <input 
                    ...
                    placeholder="pedidos@verity.com"
                    className="w-full px-4 py-3 bg-gray-50 border-none rounded-xl focus:ring-2 focus:ring-brand-orange" 
                  />
                </div>
                {index > 0 && (
