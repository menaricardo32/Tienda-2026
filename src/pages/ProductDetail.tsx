...
export default function ProductDetail() {
  ...
  const handleShareWhatsApp = () => {
    const text = `Mira esta pieza que encontré en Verity: ${product?.name}\n${productUrl}`;
    ...
    const fetchProductAndRelated = async () => {
      ...
      try {
        ...
        if (docSnap.exists()) {
          ...
          // Fetch global attributes for the variation selector
          const attrs = await getAtributos();
          ...
          const q = query(
            ...
            limit(10) // Fetch more to filter out current product
          );
          ...
          // Fetch locations to find the full address
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
          ...
          <div className="space-y-8">
            <div className="flex items-center gap-2 mb-4">
              <span className="bg-brand-orange/10 text-brand-orange text-xs font-black px-3 py-1 rounded-full uppercase tracking-widest inline-block">
                {product.category}
              </span>
              {product.subcategory && (
                <span className="bg-brand-black/10 text-brand-black text-xs font-black px-3 py-1 rounded-full uppercase tracking-widest inline-block">
                  {product.subcategory}
                </span>
              )}
            </div>
            <h1 className="text-4xl md:text-5xl font-black tracking-tighter mb-2 flex-grow">{product.name}</h1>
            ...
            {((product.hasVariations && selectedVariant) || (!product.hasVariations && product.stock !== undefined && product.stock !== null)) && (
              <div className="flex items-center space-x-2 text-xs font-bold uppercase tracking-widest mt-1">
            ...
            <div className="pt-4 space-y-4">
              {product.hasVariations && !selectedVariant && (
                <div className="flex items-center space-x-2 text-brand-orange bg-brand-orange/10 p-4 rounded-2xl">
                  <AlertCircle size={18} />
            ...
            {product.description && (
              <div className="space-y-3 pt-6 border-t border-gray-100">
                <h3 className="text-sm font-bold uppercase tracking-widest text-gray-400">Descripción</h3>
              ...
              <div className="flex flex-col gap-4">
                {product.pdfUrl && (
                  <a
                    ...
                    className="btn-outline w-full flex items-center justify-center space-x-2 border-brand-orange text-brand-orange hover:bg-brand-orange hover:text-white"
                  >
                    <FileDown size={20} />
                ...
                <p className="font-bold text-gray-400 text-sm uppercase tracking-widest mt-4">¿Te interesa este producto?</p>
                <div className="flex flex-col sm:flex-row gap-4">
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="btn-primary flex-grow flex items-center justify-center space-x-2 bg-[#25D366] hover:bg-[#128C7E]"
                  >
            ...
            {product.location && (
              <div className="space-y-4 pt-6 border-t border-gray-100 mt-6">
                ...
                <div className="bg-gray-50 p-6 rounded-2xl space-y-3">
                  <p className="font-bold text-gray-900 text-lg">{product.location}</p>
                  {(() => {
                        ...
                        <div className="space-y-4">
                          <div className="space-y-2">
                            {matchedLoc.googleMapsUrl ? (
                              <a 
                                ...
                                target="_blank" 
                                rel="noopener noreferrer"
                                className="text-gray-500 text-sm leading-relaxed hover:text-brand-orange transition-colors block"
                              >
                                {matchedLoc.address}
                              </a>
                            ) : (
                            ...
                            {matchedLoc.hours && (
                              <p className="text-brand-orange text-[10px] font-black uppercase tracking-widest mt-2">
                          ...
                          {matchedLoc.googleMapsUrl && (
                            <a 
                              ...
                              target="_blank"
                              rel="noopener noreferrer"
                              className="inline-flex items-center space-x-2 text-xs font-bold text-brand-orange bg-brand-orange/10 px-3 py-2 rounded-lg hover:bg-brand-orange hover:text-white transition-all"
                            >
